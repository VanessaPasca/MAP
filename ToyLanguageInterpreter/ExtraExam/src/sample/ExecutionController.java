...
public class ExecutionController implements Initializable {
    ...
    @FXML
    private TableView<Map.Entry<Integer,Integer>> tvLatchTable;

    @FXML
    private TableView<BarrierHelp> tvBarrierTable;
    @FXML
    ...
    @FXML
    private TableColumn<BarrierHelp,Integer> indexColumn;

    @FXML
    private TableColumn<BarrierHelp,Integer> countColumn;

    @FXML
    private TableColumn<BarrierHelp,String> threadsColumn;
    ...
    Iterable<Map.Entry<Integer,Integer>> latchTable;
    Iterable<Map.Entry<Integer, BarrierPair<List<Integer>,Integer>>> barrierTable;
    ...
    public void initProgram(PrgState state, IRepo _repo) throws InterruptedException {
        ...
        //LATCH
        latchTable=state.getLatchTable().getAll();
        List<Map.Entry<Integer,Integer>> latchList=new ArrayList<>();
        latchTable.forEach(e-> latchList.add(e));
        ObservableList<Map.Entry<Integer, Integer>> latchObs = FXCollections.observableArrayList(latchList);

        TableColumn<Map.Entry<Integer, Integer>, Integer> c1 = new TableColumn<>("Index");
        c1.setCellValueFactory(p -> (ObservableValue<Integer>)new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        TableColumn<Map.Entry<Integer, Integer>, Integer> c2 = new TableColumn<>("Count");
        c2.setCellValueFactory(p -> (ObservableValue<Integer>)new SimpleIntegerProperty(p.getValue().getValue()).asObject());

        tvLatchTable.getColumns().clear();
        tvLatchTable.getColumns().addAll(c1,c2);
        tvLatchTable.setItems(latchObs);

        //BARRIER

        barrierTable=state.getCyclicBarrier().getAll();

        List<BarrierHelp> blist=new ArrayList<>();
        barrierTable.forEach(e->blist.add(new BarrierHelp(e)));


        ObservableList<BarrierHelp> bobs=FXCollections.observableArrayList(blist);


        indexColumn.setCellValueFactory(new PropertyValueFactory<BarrierHelp,Integer>("Index"));
        countColumn.setCellValueFactory(new PropertyValueFactory<BarrierHelp,Integer>("Count"));
        threadsColumn.setCellValueFactory(new PropertyValueFactory<BarrierHelp,String>("Threads"));




        tvBarrierTable.setItems(bobs);
        ...
        btnOneStep.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                latchTable=cstate.getLatchTable().getAll();
                latchList.clear();
                latchTable.forEach(e->latchList.add(e));
                latchObs.setAll(latchList);
                tvLatchTable.setItems(latchObs);
    ...
    public class BarrierHelp{
        public int index;
        public int count;
        public String threads;

        BarrierHelp(Map.Entry<Integer,BarrierPair<List<Integer>,Integer>> entry){
            this.index=entry.getKey();
            this.count=entry.getValue().getSecond();
            threads=entry.getValue().getFirst().stream()
                    .map(e->e.toString())
                    .reduce(" ",(acc,e)->acc+e+",");
